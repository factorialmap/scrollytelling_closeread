...
Vamos fazer a simulação no R mas antes precisamos instalar ou chamar os pacotes necessários. @cr-case1_packages
...
A linha azul (com "triangulo") representa os autovalores (a importância) de cada fator extraído dos seus dados reais.
A linha vermelha (tracejada) representa a média dos autovalores que seriam esperados em um conjunto de dados aleatórios, sem estrutura real, mas com o mesmo número de variáveis e observações que o seu.
...
:::: {.cr-section layout="sidebar-left"}
 ...
 @cr-case1_fa_res Vamos explorar os resultados apresentados na análise fatorial exploratória. Usaremos a função `model_parameters` que irá extrair os parâmetros do modelo (**factor loadings**).
...
@cr-case1_results_communality Caso precise de outros resumos estatísticos como `communality` usamos aqui a função `enframe` para retornar os dados em formato de tabela. 
...
@cr-case1_results_explained O mesmo vale para a variância explicada. 
