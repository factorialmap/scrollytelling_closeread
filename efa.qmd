...
:::: {.cr-section layout="sidebar-left"}
 ...
 @cr-case1_fa_res Vamos analisar os resultados apresentados na análise fatorial exploratória. Usamos a função `model_parameters` que irá extrair os parâmetros do modelo (**factor loadings ou cargas fatoriais**), que são os coeficiêntes de correlação entre as variáveis originais(e.g. consumo online) e os fatores (e.g. ML1, ML2, ML3). Note na tabela que elas possuem um intervalo de -1 a 1. (Se aparece valores como este não se preocupe, são notações científicas que representam valores muito baixos -1.71e-03 por exemplo).
